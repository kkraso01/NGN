...
class NGNVisualizer:
    ...
    def plot_attention_heatmap(
        ...
        for i, attn in enumerate(attention_weights):
            ...
            attn_np = attn.numpy()
            if attn_np.ndim == 4:  # (batch, heads, seq_q, seq_k)
                attn_np = attn_np.mean(axis=(0, 1)).squeeze(axis=0)  # (seq_k,)
            elif attn_np.ndim == 3:  # (batch, seq_len, seq_len)
                attn_np = attn_np.mean(axis=0)

            # Plot
            if attn_np.ndim == 1:
                # Bar plot for 1D attention
                axes[i].bar(range(len(attn_np)), attn_np, color='skyblue')
    ...
    def plot_gradient_flow(
        ...
        for name, grad in gradients.items():
            if grad is not None:
                grad_norm = tf.norm(grad, ord=2).numpy()
                # Group by layer
                layer_name = name.split('/')[0]  # TF uses '/' separator
    ...
    def plot_layer_communication_patterns(
        ...
        if 'attention_weights' in debug_info:
            ...
            for attn in debug_info['attention_weights']:
                ...
                log_attn = tf.math.log(attn + 1e-8)
                entropy = -tf.reduce_sum(attn * log_attn, axis=-1)
                entropy_mean = tf.reduce_mean(entropy).numpy()
        ...
        if 'adjacency_matrix' in debug_info:
            adj = debug_info['adjacency_matrix'].numpy().flatten()
        ...
        if 'sparsity_mask' in debug_info:
            sparsity = debug_info['sparsity_mask'].numpy()
            sns.heatmap(
                sparsity,
                ax=axes[1, 0],
                cmap='Greys',
