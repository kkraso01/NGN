...
class GradientMonitor:
    ...
    def record_gradients(self) -> Dict[str, float]:
        ...
        for var in self.model.trainable_variables:
            if hasattr(var, 'grad') and var.grad is not None:
                param_norm = float(tf.norm(var.grad, ord=2).numpy())
                total_norm += param_norm ** 2
                max_norm = max(max_norm, param_norm)
                num_params += 1

                # Record per-layer norms (simplified)
                layer_name = var.name.split('/')[0] if '/' in var.name else 'model'
    ...
    def check_stability(self, threshold: float = 10.0) -> List[str]:
        ...
        for var in self.model.trainable_variables:
            if hasattr(var, 'grad') and var.grad is not None and tf.reduce_any(tf.math.is_nan(var.grad)):
                warnings.append(f"NaN gradients in variable: {var.name}")
